...
A key question emerging from our temporal evaluation is \emph{why Generation-4 protocols achieve superior security preservation over Classic PBFT}. 
...
\subsection{Why Latency Dominates Message Complexity in Temporal Security}
In our Static-Temporal Security Framework, \emph{consensus validation latency ($\tau$) exerts a significantly stronger influence on temporal survival ($P_{\text{secure}}$) than message complexity alone}. Equation (\ref{eq:p_secure_2}) shows that the temporal vulnerability decay term $W(\tau) = 1 - e^{-\lambda \tau P_{\text{TA}}}$ scales exponentially with latency $\tau$. Even if a consensus protocol achieves efficient message packaging, any queue-induced delay expands the vulnerability window $W(\tau)$, permitting stochastic attack arrivals to breach system state before transaction commitment.
