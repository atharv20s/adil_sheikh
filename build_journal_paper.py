...
For $n=51$ validator nodes and $f=16$ Byzantine faults, $OM(m)$ generates $M_{OM} \approx 2.14 \times 10^{27}$ messages per transaction block. Under practical network bandwidth limits, this exponential message complexity induces astronomical validation latency ($\tau > 43,000\text{ seconds}$). We emphasize that Lamport's $OM(m)$ algorithm serves solely as the theoretical origin of Byzantine agreement and is included to illustrate the historical evolution of Byzantine consensus rather than as a practical deployment candidate.
...
Equation (\ref{eq:latency_derivation}) combines physical network propagation delay, message complexity-dependent transmission delay, and queueing delay, where the queueing delay term $\frac{\lambda S}{\mu - \lambda S}$ is derived from classical $M/M/1$ queueing theory (e.g., Kleinrock 1975). Here $M(n)$ represents total message exchanges per consensus block:
\begin{itemize}[leftmargin=*]
